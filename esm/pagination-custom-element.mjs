...
(() => {
/*!******************************************!*\
  !*** ./src/custom-element/pagination.ts ***!
  \******************************************/
...
let PaginationElement = class PaginationElement extends HTMLElement {
    connectedCallback() {
        this.render();
        this.addEventListener('click', onClick, false)(new MutationObserver(() => this.render())).observe(this, {
            attributes: true,
    ...
    calc() {
        ...
        if (start < 1)
            (start = 1), (end = displayed);
        if (end > pages)
            (end = pages), (start = end - displayed + 1);
        return {
            current,
            pages,
            disabledFirst,
