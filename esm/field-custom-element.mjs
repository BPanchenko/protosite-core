...
let FieldElement = class FieldElement extends HTMLElement {
    connectedCallback() {
        ...
        if (this._button)
            this._button.addEventListener('click', (e) => this.clickButton(e));
        this.blur().change();
    }
    // Listeners
    blur() {
    ...
    render() {
        if (this.glyph)
            this._icon = this.renderIcon(this.glyph);
        ...
        if (this.button)
            this._button = this.renderButton({ text: this.button, type: this.buttonType });
        if (this.buttonGlyph)
            this._button = this.renderButton({ glyph: this.buttonGlyph, type: this.buttonType });
        if (this.error)
            this._error = this.renderError(this.error);
        if (this.info)
            this._info = this.renderInfo(this.info);
    ...
    set buttonGlyph(val) {
        if (this._button)
            this._button.children[0].dataset.glyph = val;
        else
            this.dataset.buttonGlyph = val;
    }
    get buttonType() {
    ...
    get type() {
        ...
        if (!type && this._field)
            type = this._field.getAttribute('type');
        return type || 'text';
    }
    set type(val) {
        // TODO: reRender this._field
        if (this._field)
            this._field.setAttribute('type', val);
        else
