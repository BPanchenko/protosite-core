{
	...
	const ORIENTATIONS = ['top-left', 'center']
	...
	customElements.define(
		...
		class extends HTMLElement {
			...
			render() {
				...
				this.#horizontal = document.createElement(
					TAGS.get('horizontal'),
				)
			...
			renderLines(container, size) {
				if (
					...
					if (diff_lines_count > 0) {
						...
						this.removeElems(
							container.children,
							old_lines_count + diff_lines_count,
						)
					}
				} else {
			...
			#positioningLines() {
				const _positioningLines = (container) => {
					...
					if (countLines % 2) {
						container.children
							.item(Math.floor(countLines / 2))
							.classList.add('s-axis')
					}
				}

				switch (this.orientation) {
					case 'top-left':
						Array.from(this.#horizontal.children).forEach(
							(elem, i) => {
								elem.style.left = i * this.step + 'px'
							},
						)

						Array.from(this.#vertical.children).forEach(
							(elem, i) => {
			...
			set orientation(value) {
				if (ORIENTATIONS.includes(value)) {
					...
					throw new Error('Orientation is not supported')
				}
			}
